...
def BMM(A, B, K, alpha, gamma):
    ...
    num_iters_gibbs = 50
    mixing_proportions = np.zeros((num_iters_gibbs,K)) # mixing proportions for all 50 iterations
    ...
    for iter in range(num_iters_gibbs):  
        for d in range(D):
            ...
            sd[d] = kk
        mixing_proportions[iter] = ((sk_docs.copy()+alpha)/np.sum(sk_docs.copy()+alpha)).flatten()
    # test documents
    lp = 0
    nd = 0
    ...
    return perplexity, swk, mixing_proportions
...
if __name__ == '__main__':
    ...
    plt.savefig("problem_d_1.pdf",bbox_inches='tight')
    I = 20
